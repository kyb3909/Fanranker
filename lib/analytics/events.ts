...
type AnalyticsEvent =
  ...
  //   motm_match = 매치센터 MoTM 탭 (2026-08-22 — 지면은 실록 한 곳뿐, 운영자 확정)
  | {
      name: "vs_impression"
      params: {
        poll_id: string
        surface: "card" | "post" | "modal" | "hero" | "motm_match"
      }
    }
  | {
      name: "vs_vote"
  ...
  // MoTM 선수 그리드 오픈 — 진입→투표 완료 전환율이 판정 지표
  | {
      name: "motm_sheet_open"
      params: { poll_id: string; surface: "motm_match" }
    }
