 ...
 * 여기만 빠르게 간다.
 *
 * ⚠️⚠️ **스코어는 이 목록이 책임진다** (2026-08-25 운영자: "스탯 풀링은 90초에 한 번씩
 *    해도 좋은데 스코어는 좀 자주 있어야 해").
 *    핵심은 두 호출의 **단가가 다르다**는 점이다:
 *      하루치 목록  = 날짜당 1콜  → 46경기가 열려도 **1콜**. 빠르게 해도 싸다.
 *      경기 상세    = 경기당 1콜  → 46경기면 46배. 빠르게 하면 비싸다.
 *    그래서 스코어를 목록 쪽에 몰고(45초) 상세는 늦춘다(90초). 스코어는 두 배 빨라지고
 *    비용은 오히려 준다 — 상세가 전체 비용의 대부분이기 때문이다.
 */
export const FRESH_DAY_LIVE_MS = 45_000
...
/**
 * 경기 상세(스탯·타임라인·분) 라이브 주기.
 * ⚠️ **경기당 1콜**이라 46경기 주말엔 이 값이 비용을 지배한다. 스코어는 하루치 목록
 *    (45초, 날짜당 1콜)이 이미 책임지므로 여기는 스탯 갱신 속도만 정한다 —
 *    운영자 판단으로 90초 (2026-08-25). 60초 대비 비용 33% 감소.
 */
export const FRESH_LIVE_MS = 90_000
