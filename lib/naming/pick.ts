 ...
 *
 * ⚠️ 이 정규식은 관심도 심사의 **축①(주체) 게이트**로도 쓰인다
 * (`app/api/cron/news-interest-filter`). 여기 걸리면 "한국 독자가 아는 팀"으로 간주돼
 * LLM 은 농도만 본다. 그래서 **맨몸 토큰이 남미 동명 구단을 잡으면 그대로 새어 나간다.**
 *
 * 실측 유출 (2026-08-15 운영자 "이상한 남미 리그들이 올라오고 있어"):
 *   "프레드, **아틀레티코 미네이루**로 이적" → `아틀레티코`(마드리드 의도)에 걸려 통과
 * 같은 함정: 인테르(→인테르나시오나우, 브라질) · 아메리카 · 나시오날 등.
 * 그래서 남미 동명 구단은 부정 전방탐색으로 떼어낸다.
 */
const SA_SUFFIX = "미네이루|파라나엔시|고이아니엔시|나시오날|투쿠만|주니오르스|나시오나우"
const CLUB_NAME_RE = new RegExp(
  [
    "리버풀|첼시|아스날|아스널|토트넘|맨체스터|맨유|맨시티|뉴캐슬|브렌트포드|에버턴|풀럼",
    "울버햄튼|울버햄프턴|브라이턴|브라이튼|본머스|번리|리즈|팰리스|빌라|웨스트햄|노팅엄|선덜랜드",
    "레알 마드리드|바르셀로나",
    // 아틀레티코·인테르는 남미 동명 구단을 배제한 뒤에만 인정
    `아틀레티코(?!\\s*(?:${SA_SUFFIX}))`,
    `인테르(?!나시오나우)`,
    "유벤투스|인터 밀란|밀란|나폴리|바이에른|뮌헨|도르트문트|레버쿠젠|라이프치히",
    "PSG|파리 생제르맹|포르투|벤피카|아약스|갈라타사라이|페네르바체|국가대표|대표팀",
  ].join("|")
)
...
/**
 * 문장에서 구단명 한 개를 뽑는다 — 네이버 검증 질의를 좁히는 재료 (2026-08-10 운영자 제안:
 * "AC밀란 선수들을 검색해서 레앙에 해당하는 이름을 찾는 식으로 검증해야지").
 *
 * 이름만 검색하면 흔한 단어가 이긴다. 실측:
 *   '레앙' 5,209 vs '레온' 45,133 → 레온 승 (오답)
 *   AC밀란 한정: '레앙' 3,384 vs '레온' 376 → 레앙 승 (정답)
 * 이적 기사는 보통 팀 두 개를 언급하는데, 어느 쪽이든 축구 맥락이라 판별에 충분하다.
 */
export function extractClubName(text: string | null | undefined): string | null {
  return text ? (CLUB_NAME_RE.exec(text)?.[0] ?? null) : null
}
