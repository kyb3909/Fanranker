 ...
 *
 * ⚠️ fail-open — 원장 쓰기가 리포트 생성을 막으면 안 된다. await 하지 않고 흘려보낸다.
 ...
 */
/**
 * 최근 N ms 안에 이 경기의 원장 행이 있나 — 크론이 "파이프라인 안쪽 게이트가 이미 사유를
 * 남겼는지" 보고, 없을 때만 바깥 게이트(경기 해석 실패)를 기록하기 위한 것.
...
export function recordReportAttempt(
  gameId: string,
  eventId: string | null,
  stage: ReportStage,
  reason?: string | null
