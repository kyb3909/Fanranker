...
0. **원문에 없는 내용** (원문이 주어졌을 때만 검사) — 이게 가장 중요하다.
   ...
   ✅ 통과 — **숫자 표기 변환은 새 내용이 아니다.** 이걸 반드시 알아봐라:
        ...
        "on the hour mark" = "경기 60분" = "후반 15분" (축구 전후반 각 45분 기준)
...
4. 무내용 — 원문에서 옮긴 **사실이 없고 원문을 소개만** 하는 글. "자세한 내용은 기사에서
   ...
   없는 경우
5. 인터뷰 맥락 왜곡 — 공식 홈페이지 인터뷰·기자회견에도 동일하게 적용한다.
   · 기사에서 핵심으로 다룬 평가·결정의 이유가 원문에 명시돼 있는데, 그 이유나 조건을
     빼서 발언의 뜻을 이해할 수 없거나 다르게 이해하게 만든 경우. 예: '수적 열세가
     상대에게도 어려웠다'만 쓰고, 그 이유인 역습 공간 차단 설명을 누락한 것.
     이처럼 원래 불리한 상황이 상대에게도 불리하다는 역설적 평가는 본문에 인용한
     이상 그 근거가 필요하다. 제목의 주제가 승리나 믿음이어도 이 검사를 생략하지
     않는다. 원문에 있는 이유가 기사에 전혀 없으면 맥락 누락으로 불통과다.
   · 기자의 질문을 답변자의 주장으로, 구단 기자의 서술을 직접 발언으로, 발언자가
     제4심에게 들었다는 전언을 심판진의 공식 발표나 오심 인정으로 바꾼 경우.
   · 경기 전/후 발언을 뒤바꾸거나, 과거 발언을 이번 발언으로 붙이거나, 원문에 없는
     질문을 만들어 붙인 경우. 게시일을 근거 없이 발언일로 단정한 경우.
   · 화자·대상·부정·조건이 달라졌거나, 관용구의 직역으로 원래 뜻을 전달하지 못한 경우.
   ✅ 통과: 핵심 의미와 필요한 이유를 보존한 간접 인용·자연스러운 의역·주제에 맞는
      답변 선별. 인터뷰의 모든 질문·답변을 담을 필요는 없다. 원문에 없는 질문이나
      이유를 채우라고 요구하지 않는다. 경미한 문체 취향만으로 반려하지 않는다.
   불통과 사유에는 문제가 되는 기사 구절과 원문의 대응 의미를 구체적으로 적는다.
