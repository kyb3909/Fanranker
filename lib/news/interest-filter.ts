...
## 축① 주체 — 한국 독자가 아는 대상인가
### 리그별 커버 범위 (2026-08-25 운영자 확정 — 이 목록이 기준이다)
아래에 없으면 **그 리그 안에서 끝나는 이야기는 소음**이다.

| 리그 | 범위 |
|---|---|
| 프리미어리그 | **전 구단** (승격팀·하위권 포함) |
| 프리메라리가(라리가) | **전 구단** |
| 챔피언십(잉글랜드 2부) | **전 구단** — 한국 선수가 많다 |
| 분데스리가 | **바이에른 뮌헨 · 도르트문트 · 라이프치히**, 그리고 한국 선수 |
| 세리에A | **나폴리 · 밀란 · 인테르 · 유벤투스 · 코모 · 로마 · 라치오 · 아탈란타** |
| 리그1 | **PSG · 모나코 · 리옹 · 스트라스부르**, 그리고 한국 선수 |

### ⚠️⚠️ 한쪽만 걸려도 통과다 (2026-08-25 운영자)
이적·경기·협상은 **양쪽 당사자 중 하나만** 커버 대상이면 통과다. 상대가 누구든 상관없다.
  · "애스턴 빌라, 웨스트햄 완비사카 영입 합의" → 둘 다 EPL. **통과**
  · "리즈, 첼시 무드리크 임대 추진" → 둘 다 커버. **통과**
  · "나폴리, 세르비아 리그 유망주 영입" → 나폴리가 걸린다. **통과**
  · "알 힐랄, 마르티넬리 영입 임박" → 사우디 리그지만 마르티넬리(아스날)가 걸린다. **통과**
상대가 커버 밖이라는 이유로 반려하지 마라 — 걸리는 쪽 하나를 찾으면 그걸로 끝이다.

⚠️ **구단 제한은 분데스리가·세리에A·리그1 세 리그에만 적용된다.**
  프리미어리그·라리가·챔피언십은 **구단을 따지지 마라 — 전 구단이 통과다.**
  (실측 오반려: "애스턴 빌라, 웨스트햄 완비사카 영입 합의" 를 "웨스트햄은 커버 대상이
  아니"라며 버렸다. 둘 다 EPL 이다. 이 세 리그에서 구단명을 이유로 반려하면 그건 오답이다.)

⚠️ 위 세 리그에 한해, 표에 없는 구단이 **주체**면 반려다. 분데스리가라고, 세리에A라고 통과가 아니다
  (예: "볼프스부르크, 감독 경질" → drop / "프라이부르크 미드필더 이적" → drop).
  단 그 구단이 **위 표의 구단·선수와 얽힌** 이야기면 통과다
  (예: "프라이부르크, 뮌헨전 2-1 승" 은 뮌헨이 주체로 얽혔으므로 keep).

### 리그와 무관하게 통과
- **한국인 선수 — 무조건.** 어느 리그, 어느 디비전이든.
- **축구 협회 뉴스 — 매우 중요.** FIFA·UEFA·AFC·대한축구협회의 결정, 규정 변경,
  징계, 대회 방식·일정 변경, 월드컵 관련. 이건 반려하지 마라.
- **국가대표 — 한국·일본, 그리고 아시아 국가대표 전반.** 소집·평가전·예선·감독 선임.
- **이름이 알려진 선수·감독** (아래 "유명" 정의 참조)
- **선수·감독의 사망, 생명이 위중한 사고·질환 — 무조건 통과** (2026-08-25 운영자).
  리그도 유명도도 따지지 마라. 하부리그의 이름 모를 선수라도 마찬가지다.
  · "포르투갈 하부리그 선수, 친선 경기 중 사망" → **통과** (실측 오반려 사례)
  ⚠️ 아래 "무명 주체" 규칙보다 **이 규칙이 우선한다.**

⚠️ **유명 클럽과 유명 선수는 각각 독립된 관심 요소다. 하나만 걸려도 통과.**
유명 선수는 **어느 리그에 있든** 계속 조명한다 — 이적설이든 그 뒤의 활약이든 마찬가지다.
  · "트라브존스포르, 살라 영입 추진" → **살라**가 걸리므로 통과
  · "살라, 터키 리그 데뷔전 2골" → 리그가 마이너여도 **살라**가 걸리므로 통과

⚠️ 단, 여기서 "유명"은 **지금 유럽 빅리그에서 뛰는 급**을 말한다. **한때 유명했던 선수는
유명이 아니다.** 유럽을 떠나 남미·중동·아시아 리그로 간 선수의 그 뒤 소식은, 본인이
살라·홀란급 현역 월드클래스가 아닌 한 반려다.
  · "가브리엘 바르보사(가비골), 산투스에서 100골" → **drop** (브라질 리그, 지금은 유럽 밖)
  · "프레드, 아틀레티코 미네이루로 이적" → **drop** (전 맨유지만 지금은 남미 리그 간 이적)

반려:
- **남미 리그 (브라질 세리에A·아르헨티나·기타 남미) — 리그 내부 소식은 전부.**
  구단 간 이적, 경기 활약, 기록, 구단 성명, 팬 사건까지 리그 안에서 끝나는 이야기는 반려다.
  코린치안스·플라멩구·산투스·보카·리베르 같은 이름난 구단이라도 마찬가지 — 한국 독자는
  남미 리그를 소비하지 않는다.
  ⚠️ **예외: 유럽 빅클럽이 주체로 걸리면 통과.** "레알, 플라멩구 유망주에 4천만 유로" 는
  주체가 레알이므로 keep 이다. 가르는 기준은 **유럽 빅리그로 오는 이야기냐, 남미 안에서
  끝나는 이야기냐**다.
- 하부리그·마이너리그 (프랑스 2부, 하부 디비전 등) — 구단 존폐·청산이라도 주체가 안 걸리면 drop
  ⚠️ **잉글랜드 챔피언십은 예외 — 하부리그가 아니라 커버 대상이다** (위 표).
- 위 표에 없는 리그 (포르투갈·네덜란드·튀르키예·벨기에·스코틀랜드·MLS·사우디 등)의
  **리그 내부 소식** — 유명 선수나 한국 선수가 걸리지 않으면 drop
- **유럽 국가대표 소식** (독일·프랑스 대표팀 소집 등) — 우리 독자의 관심 밖이다.
  아시아 대표팀과는 다르게 취급하라
