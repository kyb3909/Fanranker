...
const STEPS = [
  {
    num: "01",
    title: "진영 선택 참가",
    body: "첼시 팬과 리버풀 팬, 두 진영 중 하나를 골라 참가합니다. 다른 팀 팬도 둘 중 하나를 골라 참전할 수 있어요. 이벤트 중 진영 변경은 불가.",
  },
  {
    num: "02",
    title: "매일 예측",
    body: "매일 충전되는 무료 볼 10개로 경기를 예측합니다. 한 예측에 볼 1~10개를 걸 수 있어요 — 어디에 몰지가 전략입니다.",
  },
  {
    num: "03",
    title: "라운드 결과 발표",
    body: "라운드마다 이긴 팬덤을 가리고, 승리 팬덤에서 추첨으로 유니폼 주인공이 나옵니다. 집계는 채널 공개 시점부터 시작돼요.",
  },
] as const

/**
 * 상품 구성 — **준비가 끝난 것만 적는다.**
 * 스팀 기프트카드(시즌 오픈 이벤트 상품)는 이 대결에 걸려 있지 않으므로 뺐다.
 * 클롭 모자·테리 완장은 실물이 준비돼 있으나 배분 규칙이 미정이라 수량만 적고
 * 지급 방식은 "추후 공지"로 남긴다 — 안 도는 걸 확정처럼 쓰지 않는다.
 */
const PRIZES = [
  [
    "라운드 승리 팬덤",
    "이번 시즌 유니폼 — 승리 팬덤이 고른 팀 것",
    "라운드마다 1명",
    "승리 팬덤 참가자 중 추첨",
  ],
  ["위르겐 클롭 사인 모자", "실물 준비 완료", "1개", "지급 방식은 추후 공지"],
  ["존 테리 사인 주장 완장", "실물 준비 완료", "1개", "지급 방식은 추후 공지"],
] as const

const NOTICES = [
  "한 번 선택한 진영은 이벤트가 끝날 때까지 변경할 수 없습니다. 이기고 있는 쪽으로 갈아타는 것을 막기 위한 규칙입니다.",
  "계정당 한 번만 참가할 수 있고, 참가 취소와 재등록은 되지 않습니다.",
  "라운드 성적 집계는 코그 채널 공개 시점부터 시작됩니다. 그 전에 한 예측은 대결 성적에 들어가지 않습니다.",
  "유니폼은 라운드 승리 팬덤의 참가자 중 추첨으로 정해집니다 — 성적 1위에게 확정 지급되는 상품이 아닙니다.",
  "클롭 사인 모자와 테리 사인 완장은 실물이 준비돼 있으나, 지급 시점과 방식은 채널 공개에 맞춰 따로 공지합니다.",
  "이벤트 기간 중 삭제된 글·댓글은 활동 기록에서 제외되며, 무성의한 도배는 검수 후 당첨이 취소될 수 있습니다.",
    ...
    <div className="min-h-screen" style={{ background: "var(--wc-paper)" }}>
      ...
      <main className="mx-auto w-full max-w-[1080px] px-4 pt-9 sm:px-6">
        ...
        >
          {[
            ["대결 기간", "약 4주", period],
            ["참여 방법", "예측하고, 떠들어라", "매일 채워지는 무료 볼 10개로 경기를 예측"],
            ["보상 안내", "승리 팬덤에 유니폼", "클롭 사인 모자·테리 사인 완장도 실물 준비 완료"],
          ].map(([k, t, d]) => (
        ...
        <section className="mt-12" aria-label="크리에이터 맞대결 — 유니폼의 행방">
          ...
          >
            ...
            <div className="absolute right-0 bottom-0 left-0 px-5 pb-6 text-center sm:pb-8">
              ...
              >
                첼루키와 리빅이 라운드마다 시청자 팬덤을 걸고 맞붙습니다. 이긴 쪽 팬덤에서 유니폼
        ...
        <section className="mt-12">
          <div className="mb-[20px] text-center">
            ...
            <p className="mt-[10px] text-[14.5px]" style={{ color: "var(--wc-mute)" }}>
              유니폼은 라운드 승리 팬덤의 참가자 중 추첨으로 정해집니다. 사인 상품의 지급 방식은
              채널 공개에 맞춰 따로 공지합니다.
            </p>
          </div>
          <div
