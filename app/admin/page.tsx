...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    supabase.from('betman_games').select('*', { count: 'exact', head: true }).in('status', ['scheduled', 'in_progress']),
    supabase.from('content_reports').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
    supabase.from('betman_sync_state').select('last_checked_at, latest_gm_ts, last_sync_action').order('updated_at', { ascending: false }).limit(1).maybeSingle(),
    supabase.from('news_ticker_items').select('created_at').order('created_at', { ascending: false }).limit(1).maybeSingle(),
    supabase.from('news_ticker_items').select('*', { count: 'exact', head: true }).gte('created_at', new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString()),
    supabase.from('betman_daily_rounds').select('daily_id, bet_close_at, status').eq('status', 'open').order('bet_close_at', { ascending: false }).limit(1).maybeSingle(),
  ])
