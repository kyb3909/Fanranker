...
const CertifyJournalistSchema = z.object({
 ...
 *
 * Manually certify a user as a journalist (admin only)
 *
 * Body:
 * - user_id: string (required) - User ID to certify
 * - revoke?: boolean - If true, revoke journalist status instead of granting
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const updateData: {
      ...
      is_journalist: !revoke,
      updated_at: new Date().toISOString(),
    }

    if (!revoke && !profile.is_journalist) {
      updateData.journalist_certified_at = new Date().toISOString()
    } else if (revoke) {
      updateData.journalist_certified_at = null
    }
    ...
    if (updateError) {
      return apiError("기자 인증 상태 업데이트 중 오류가 발생했습니다.", 500, updateError)
    }
    ...
    await supabaseAudit.from("admin_audit_logs").insert({
      ...
      action: revoke ? "revoke_journalist" : "certify_journalist",
      target_type: "user",
      target_id: user_id,
    ...
    return NextResponse.json({
      success: true,
      message: revoke ? "기자 인증이 해제되었습니다." : "기자 인증이 완료되었습니다.",
