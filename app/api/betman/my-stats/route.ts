...
export async function GET() {
  try {
    ...
    const { data: stats, error } = await supabase
      ...
      .select(
        "user_id, sport, total_predictions, correct_predictions, wrong_predictions, cancelled_predictions, accuracy, total_wagered, total_returns, net_profit, profit_rate, current_streak, best_win_streak, worst_lose_streak"
      )
      .eq("user_id", userId)
      .order("sport")
