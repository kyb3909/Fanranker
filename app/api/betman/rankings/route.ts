...
export async function GET(request: NextRequest) {
  try {
    ...
    // 정렬 컬럼 매핑
    const sortColumn = sort === 'accuracy' ? 'accuracy'
      : sort === 'net_profit' ? 'net_profit'
      : 'profit_rate'
      ...
      if (userId) {
        ...
        if (myStats && (myStats.total_wagered ?? 0) > 0) {
          ...
          const myProfile = profileMap.get(userId)
            || (await supabase.from('profiles').select('nickname, avatar_url').eq('user_id', userId).single()).data
