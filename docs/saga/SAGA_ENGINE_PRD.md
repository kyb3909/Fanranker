...
| D15 | **9/1 대량 정산 없음 (2026-08-06 오너 확정).** 데드라인 데이는 saga-deadline의 로그 종결(stage별 outcome 기록)만 수행 — 포인트 일괄 정산·예언자 호칭·소환 v0 전부 취소. 스키마(saga_settlements·saga_settled 알림)는 향후 재개 대비로 유지 | 2026-08-04 운영자 "정산·소환 필요 없어, 로그만 존재하면 돼"가 코드 주석에만 있던 것을 2026-08-06 점검(F3)에서 확인 요청 → 오너 확정. 본 문서 §4.2·§7·§9의 정산 서술은 이 결정으로 취소됨 |
| D16 | **경기 데이터 수집 = Soccerway headless 크롤 우선 + 무료 API 교차 보정 (2026-08-06 오너 확정 — D11 개정).** 매치 발견·매핑은 정적 날짜 페이지(빌라-뮌헨 실측: 정적으로 매치 URL·팀 해시 확보), 라인업·이벤트 상세는 headless(기존 match-preview 인프라 재활용, VPS). FT 후 무료 API 1~2회 호출로 스코어·득점자 대조, 불일치는 자동 수정 없이 검수 큐. 평점은 팬 평점(자체) 선행 — 데이터 평점은 무료 API 또는 자체 계산 후속 | 오너 비용 판단(2026-08-06): 유료 API 대신 크롤+무료 보정. D11의 근거(ToS·유지비)는 유효한 위험으로 남으므로 마크업 파손·차단 시 API 전환을 폴백으로 유지. match-preview PRD의 soccerway 인프라는 폐기가 아니라 이 결정의 기반이 됨. (2026-08-07 실측 개정: 신 soccerway 전면 SPA — 발견은 날짜 페이지가 아니라 팀 해시 구성 URL 정적 대조. 종료 경기 데이터에 mid 불요, 소커웨이 자체 선수 평점 존재 — missing-information I-3b) |
| D17 | **경기 리뷰 자동 생성 = 소커웨이 리포트 참고 허용 + "요약 4문단+선발 라인업" 카드 → 시즌 연표 (2026-08-07 오너 확정 — D14 개정).** 추출 데이터(스코어·득점·하프별·스탯·라인업)를 뼈대로, 소커웨이 리포트 기사(정적 fetch 가능 실측)는 맥락 소스로만 참고해 드라이 톤 한국어 재작성. 그대로 번역 게재 금지. 리뷰 카드는 이적 사가 메뉴의 카드·연표 UI 패턴을 재사용해 시즌 연표(실록)로 편찬 — "하나하나 카드 뉴스가 되어 연표를 이적 사가 메뉴처럼" (오너). 발행은 기존 뉴스룸 게이트(표기 사전+네이버 검증 루프·검사관·이미지) 전량 통과 후 | 오너 지시 2026-08-07 "매치 리포트를 한국어로 변환·요약" + "라인업 정도만 있으면 딱 좋을 것" + 카드·연표 완성상. D14의 "외부 텍스트 금지"는 리뷰 본문을 외부 기사로 채우는 것의 금지로 좁혀짐 — 데이터 우선 재작성 원칙은 유지. 페네르-슈투름 2-0 데모로 템플릿 확정 (EVIDENCE_stage3_rehearsal) |
...
- ~~정산 규칙: `done` → "나간다" 승 / 마감 시 `stayed` → "남는다" 승~~ **(D15로 취소)**
- ~~**8/31 대량 정산**: 열린 사가 전체 강제 종결 → 포인트 일괄 정산 → 9/1 "이적시장 예언자" 호칭 시상 → 리더보드가 시즌 예측으로 유저를 인계~~ **(D15로 취소 — 마감은 로그 종결만: done→'done', 미성사→'stayed')**
...
- ~~**호칭 연동**: 8/31 정산 → "이적시장 예언자" 등. 기존 호칭 시스템 사용 `[P0확인]`~~ **(D15로 취소)**
...
| **A5 종결** | `stayed`·`done` 자동 종결 잡 (saga-deadline — 로그만) | ~~대량 정산 / 예언자 호칭 / 종결 리포트 / 소환~~ (D15로 취소) |

게이트: EPIC별 Playwright 스모크(문서 생성→이벤트 append→범프→투표→댓글→종결 리포트) + 마이그레이션 리허설.

주차 스케치: **W1~2** A1+A2 → **W3** A3 → **W4** A4+A5+데드라인 모드 → **8/31 마감(로그 종결만, D15)** → 9월 Phase B.
...
4. ~~대량 정산 기준 시각~~ → ~~오너 확정(2026-08-03): 9/1 09:00 KST~~ → **오너 재확정(2026-08-06): 정산 자체 취소 (D15).** 마감 시각 9/1 09:00 KST 는 로그 종결 기준으로만 유지
...
- 현재 페이즈: Phase A (TransferSaga, ~2026-08-31 데드라인 데이 — 정산 없음, 로그 종결만. D15)
