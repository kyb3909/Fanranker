 ...
 *     운영자 방침(2026-08-12): 혜택은 **수단(볼)이 아니라 결과(점수)로** 말한다 —
 *     "승부예측으로 점수를 쌓아보세요". 이건 코인 경제의 오랜 원칙과도 같다:
 *     적중해도 화폐로 보상하지 않고 점수로만 남긴다(lib/betman/settle.ts "점수만 모델").
 *     화폐를 앞세우는 순간 그 원칙이 화면에서 뒤집힌다.
 *
 *     ⚠️ "무료 볼 10개" 같은 **규칙 설명**은 금지 대상이 아니다(월드컵·시즌 참여 안내).
 *     막는 건 "무료 10볼"처럼 지급물을 앞세운 유인 문구다 — 그래서 어순으로 가른다.
 *
 * ⚠️ 이 패턴들은 좁게 잡았다. '걸다'는 일상어라("전화를 걸다", "링크를 걸다")
 * 넓게 잡으면 오탐이 쏟아진다. 그래서 세 가지 꼴만 잡는다:
 *   · 픽·볼이 목적어로 붙은 '걸다'        — "픽을 걸", "볼 걸"
 *   · 목적어가 생략된 판돈 관용구          — "안 걸면", "걸러 가"
 *   · 지급물을 앞세운 유인 문구            — "무료 10볼", "무료 10개"
 * 실제로 이 저장소에 있던 옛 문구 6종으로 검증했다(4종만 잡히던 초안을 넓힌 결과).
 */
const BANNED_FRAMING = /(픽|볼)\s*(을|를)?\s*걸|안\s*걸(면|고)|걸러\s*가|무료\s*\d+\s*(볼|개)/
