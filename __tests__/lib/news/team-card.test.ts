...
describe("resolveTeamCard — 겹치는 구단명을 정확히 가른다", () => {
  ...
  /**
   * ⚠️ 이 테스트는 원래 "카드가 없는 구단(에버턴·풀럼)은 중립으로 떨어진다" 였다.
   *    2026-08-20 에셋 보강(`aedc8005`)으로 **사전에 등록된 전 구단이 카드를 갖게 되면서**
   *    그 상황 자체가 사라졌고, 테스트만 낡은 전제를 붙들고 실패하고 있었다.
   *
   *    구단을 바꿔 끼우는 건 답이 아니다 — 카드 없는 구단이 **하나도 없다**.
   *    그래서 지금 실제로 지킬 가치가 있는 불변식으로 바꾼다:
   *    **매핑에 구단을 추가하면 카드 파일도 같이 추가해야 한다.**
   *    이쪽이 원본보다 강하다. 파일을 빠뜨린 매핑을 잡아내기 때문이다.
   */
  it("⭐매핑된 구단은 전부 실제 카드 파일을 갖는다", () => {
    const clubs = [
      "아스날",
      "리버풀",
      "맨체스터 시티",
    ...
    const missing = clubs
      .map((c) => ({ c, card: resolveTeamCard(`${c} 소식`) }))
      .filter(({ card }) => card !== DEFAULT_TEAM_CARD)
      .filter(({ card }) => !existsSync(join(process.cwd(), "public", card)))
      .map(({ c, card }) => `${c} → ${card}`)

    expect(missing, "매핑은 있는데 카드 파일이 없다 — public/images/news-team/ 확인").toEqual([])
  })

  it("사전에 없는 구단은 중립으로 떨어진다", () => {
    expect(resolveTeamCard("울버햄튼, 수비수 임대 영입")).toBe(DEFAULT_TEAM_CARD)
  })
