...
describe("resolveTeamCard — 겹치는 구단명을 정확히 가른다", () => {
   ...
   *    2026-08-20 에셋 보강(`aedc8005`)으로 **사전 등재 전 구단이 카드를 갖게 되면서**
   *    그 상황 자체가 사라졌고(실측: 매핑 대비 누락 0), 테스트만 낡은 전제를 붙들고
   *    실패하고 있었다. 구단을 바꿔 끼우는 건 답이 아니다 — 카드 없는 구단이 없다.
   *
   *    "매핑된 구단은 카드 파일을 갖는다" 는 불변식은 **아래 describe 가 이미 검사한다**
   *    (`모든 매핑 대상 카드 파일이 실제로 존재한다`). 그래서 여기서는 중복하지 않고,
   *    이 자리에 남길 가치가 있는 것 — **사전에 아예 없는 구단의 폴백** — 만 검사한다.
   */
