...
sync_one_gmts() {
  ...
  # 게임 파싱 — 2026-05 betman 신 매핑 적용.
  # betTypId 분류:
  #   1=승무패(3-way), 2=승패2way(2-way 무승부 X), 3=승1패(BS) / 승5패(BK) (3-way),
  #   4=핸디캡(3-way), 5=소수핸디캡(2-way), 7=언더오버, 9=SUM(홀짝)
  local JQ_FILTER='
. as $root |
...
[ $root.compSchedules.datas[] |
  ...
  ({"1":"일반","2":"승패2way","3":"승1패","4":"핸디캡","5":"소수핸디캡","7":"언더오버","9":"SUM","13":"소수핸디캡"}[$d[$idx.betTypId] | tostring]) as $gt_raw |
  select($gt_raw != null) |
  ({"SC":"축구","BK":"농구","VL":"배구","BS":"야구"}[$d[$idx.itemCode] // ""] // ($d[$idx.itemCode] // "축구")) as $sp |
  # 농구의 betTypId=3 은 승5패 (5점차 미만 무승부)
  (if $gt_raw == "승1패" and $d[$idx.itemCode] == "BK" then "승5패" else $gt_raw end) as $gt |
  ($d[$idx.winAllot] // 0) as $wa |
  ($d[$idx.drawAllot] // 0) as $da |
  ($d[$idx.loseAllot] // 0) as $la |
  ($d[$idx.winHandi] // 0) as $wh |
  # 3-way (홈/무/원) 마켓 — odds 모두 채움
  ($gt == "일반" or $gt == "핸디캡" or $gt == "승1패" or $gt == "승5패") as $is_3way |
  # 2-way (홈/원) 마켓 — draw 없음
  ($gt == "승패2way" or $gt == "소수핸디캡") as $is_2way |
  {
    round_id: $rid,
    ...
    draw_odds: (if $is_3way and $da > 0 then $da else null end),
    away_win_odds: (if ($is_3way or $is_2way) and $la > 0 then $la else null end),
...
[ $root.compSchedules.datas[] |
  . as $d |
  select(($d[$idx.winAllot] // 0) != 0 or ($d[$idx.drawAllot] // 0) != 0 or ($d[$idx.loseAllot] // 0) != 0) |
  ({"1":"일반","2":"승패2way","3":"승1패","4":"핸디캡","5":"소수핸디캡","7":"언더오버","9":"SUM","13":"소수핸디캡"}[$d[$idx.betTypId] | tostring]) as $gt |
  select($gt == null) |
