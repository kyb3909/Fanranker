...
for GMTS in $GMTS_LIST; do
  ...
  # ── jq로 결과 파싱 + 실제 점수 매핑 ──
  # HANDI_VAL: 0,14=일반, 2=핸디캡, 9=언더오버, 5=SUM
  # GAME_RESULT: 0=WIN(홈승/언더), 1=DRAW, 2=LOSE(원정승/오버), 4=취소
  PARSED=$(echo "$RESULT_RESP" | jq '
    # 1단계: 일반 게임에서 실제 점수 맵 구축
    (.detlBody | map(select((.HANDI_VAL // 0) == 0 or (.HANDI_VAL // 0) == 14))
     | map({
    ...
    [.detlBody[] |
      ...
      # 점수: 일반은 MCH_SCORE에서, 핸디캡/언오버는 일반 게임 점수 참조
  ...
  UNKNOWN_RESULTS=$(echo "$RESULT_RESP" | jq --arg gmts "$GMTS" '
    [ .detlBody[] |
      . as $item |
      ((.HANDI_VAL // 0) | tostring) as $hv |
      ({"0":"일반","14":"일반","2":"핸디캡","9":"언더오버","5":"SUM","6":"S핸디캡","7":"S언더오버"}[$hv]) as $gt_known |
      (.GAME_RESULT // "" | tostring) as $gr |
      (if $gr == "4" then "cancelled"
