...
async function main() {
  ...
  const GENRE: [string, RegExp][] = [
    ["이적 루머(관심·검토·경쟁)", /관심|검토|루머|거론|연결|경쟁|추진|주시|고려/],
    ["제3자 논평·발언", /논평|발언|언급|소감|인터뷰|평가/],
    ["재탕·과거사", /재탕|과거|이미 알려/],
    ["행정·규정·정치", /규정|행정|협회|징계|FIFA|UEFA|심판|정치/],
    ["신변잡기·잡담", /잡담|신변|일정|이동|SNS|목격/],
    ["무명 주체", /무명|하부|마이너|관심 낮|알려지지/],
  ]
  ...
  rows
    .map((r, i) => ({ r, i }))
    .filter(({ r, i }) => verdicts[i] === false && (r.view_count ?? 0) === 0)
    .slice(0, 12)
