...
type Article = { kind: "post" | "draft"; id: string; title: string; created_at: string }
type Articles = { items: Article[]; limit: number }
...
export function DeskArticlePicker({
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR<Articles>(
    open ? `${API}?q=${encodeURIComponent(search)}` : null,
    loadArticles,
  ...
  const items = data?.items.filter((article) => kind === "all" || article.kind === kind) ?? []
  const busy = loadingId !== null
  async function choose(article: Article) {
    ...
    <section className="bg-card overflow-hidden rounded-xl border" aria-label="실제 기사 선택">
      ...
      >
        <span className="flex items-center gap-3">
          ...
          <span>
            <span className="block font-semibold">올라온 기사 골라 데스킹</span>
            <span className="text-muted-foreground mt-1 block text-xs">
              현재 제목·본문 그대로 가져와 직접 수정합니다.
            </span>
          </span>
        </span>
        <ChevronDown
      ...
      <div id="desk-article-list" hidden={!open} className="space-y-3 border-t p-4 sm:p-5">
          ...
          <>
                    ...
                    >
                      <span className="min-w-0 flex-1">
                        <span className="mb-1.5 flex flex-wrap items-center gap-2 text-xs">
                          ...
                          >
                            {article.kind === "post" ? "발행된 기사" : "발행 대기"}
                          </span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <p className="text-muted-foreground">
                최근 기사 중 최대 {data?.limit ?? 80}건에서 선택 · 현재 목록 {items.length}건
              </p>
              <Button
                size="sm"
                variant="ghost"
        ...
        <p className="text-muted-foreground text-xs leading-5">
          가져온 기사의 교정은 학습용 사본에 저장됩니다. 공개된 기사는 목록의 새 창 아이콘으로
          확인할 수 있습니다.
        </p>
      </div>
    </section>
